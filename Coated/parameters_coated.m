function p = parameters_coated(varargin)
    ...
    if nargin == 0
        ...
        p.P_R = 1.5*p.vap_press_MeOH; %Sytem presure 50% higher than vapor pressure of Methanol (atm) taken 
        ...
        p.eps_p = 0.4; % porocity 
        p.tau_p = 4; % tortuosity
        p.L_sio = 42.8e3; % Bronsted sites (mol_SiO/m2_cat)
        p.Deff = p.eps_p/p.tau_p*p.D_ME; % Diffusivites to the surface of the catalyst (m2/s)
        p.L_coat = 400e-6; % Thickness of the coating (m)
        p.V_solid = pi*p.rd^2*p.L_coat; % Volume of the coating solid (m3)

        %% Volume and Holdups
        p.V_r = 6.2e-5; % m3
        p.Fv_Tot = p.Fv_MeOH+p.Fv_TG; % total volumetric flow (m3_r/s)
        p.eps_TG = p.Fv_TG / p.Fv_Tot; % epsilon TG
        p.eps_MeOH = p.Fv_MeOH / p.Fv_Tot; % epsilon MeOH
        p.eps_s = p.V_solid/p.V_r; % epsilon solid
        ...
        p.k_het_ref = [0.0842, 0.1273, 0.1032, 0.0232, 0.0571, 0.0083]/60/1000000; % (m3/mol/s)/m2
        ...
        %p.k_het = [0.1172, 0.1680, 0.1372, 0.0565, 0.0870, 0.018]/60*7.04;
