...
p.tspan = [0 20]; % Simulation Time(s)
...
K_OV_LL_all = zeros(length(rot_speed), 6); % Liquid–liquid MTCs for all species
species_labels = {'TG', 'DG', 'MG', 'G', 'FAME', 'MeOH'};
...
for i = 1:length(rot_speed)
    ...
    p.dagg = (p.dp + 5.76e3*p.spe_enrgy^(-2/5)*p.dp); % Average diameter of agglomerates (m)
    p.Vagg = pi/6*p.dagg^3; % Volume of the agglomerates (m3_agg)
    p.Nagg = p.eps_s*p.V_r/p.Vagg; % Number of agglomerates in the system
    p.davg = (p.dagg^3 + 6*(p.V_MeOH - p.V_pores)/p.Nagg)^(1/3); % Average Droplet size (m)
    p.davg_hom = 0.062*(p.ST/p.rho_TG)^(3/5)*p.spe_enrgy^(-2/5); % Average Droplet size for homogenous system for comparison (m)
    d_avg(i) = p.davg; % storing d_avg for each
    d_avg_hom(i) = p.davg_hom; % storing d_avg_hom
    d_agg(i) = p.dagg; % storing d_agg
    ...
    p.Sh_solid = 2; % For Solid Phase
    p.K_s = (p.Sh_solid*p.Deff)./p.dp; %  Mass transfer coefficient of components in ME (m3_i_ME/m2_int.s)
    p.K_ME_S = (p.Sh_MeOH*p.D_ME)./p.dp; %  Mass transfer coefficient of components in ME (m3_i_ME/m2_int.s)
    ...
    p.K_OV_LS = 1./((1./p.K_ME_S) + (1./p.K_s)); % Overall Mass Transfer Coefficients (m3_i_TG/m2_int .s)
    ...
    K_OV_LL_all(i, :) = p.K_OV_LL; % storing the mass transfer coeff for all components
...
% Plotting the Effect of Rotation speed on d_agg
figure;
plot(rot_speed,d_agg, 'o--',color='red')
xlabel('Rotation speed (rad/s)')
ylabel('Agglomeration size (m)')
title('Effect of Rotation speed on dagg ')
%saveas(gcf, fullfile(outputFolder, 'dagg_vs_RotationSpeed.png'));

% Plotting the Effect of Rotation speed on davg_hom
figure;
plot(rot_speed,d_avg_hom, 'o--',color='red')
xlabel('Rotation speed (rad/s)')
ylabel('Average droplet size (m)')
title('Effect of Rotation speed on davg homogenous')
%saveas(gcf, fullfile(outputFolder, 'davg_hom_vs_RotationSpeed.png'));
...
%if exist('X_vs_catload.mat', 'file')
%    load('X_vs_catload.mat', 'cat_loading', 'conversion_X');
%else
%    cat_loading = [];
%    conversion_X = [];
%end
...
%cat_loading(end+1) = p.wt_cat;
%conversion_X(end+1) = X(end); % or max(X) depending on what you want
%save('X_vs_catload.mat', 'cat_loading', 'conversion_X');
...
% Plot cat loading vs conversion
load('X_vs_catload.mat', 'cat_loading', 'conversion_X');

figure;
plot(cat_loading, conversion_X, 'o--', 'LineWidth', 2);
xlabel('Catalyst loading (wt/wt TG)');
ylabel('TG conversion');
title('Effect of Catalyst Loading on Conversion');
grid on;
...
% Plotting K_OV_LL for all components
figure;
hold on;
for j = 1:6
    plot(rot_speed, K_OV_LL_all(:,j), 'LineWidth', 1.5, 'DisplayName', species_labels{j});
end
xlabel('Rotation speed (rad/s)');
ylabel('K_{OV,LL} (m/s)');
title('Overall Mass Transfer Coefficients at Liquid–Liquid Interface');
legend('Location', 'best');
grid on;
%saveas(gcf, fullfile(outputFolder, 'k_OV_LL_vs_omega.png'));

% Plotting K_{OV,LL} for TG only
figure;
plot(rot_speed, K_OV_LL_all(:,1), 'o-', 'LineWidth', 2, 'Color', 'blue');
xlabel('Rotation speed (rad/s)');
ylabel('K_{OV,LL} for TG (m/s)');
title('Effect of Rotation Speed on K_{OV,LL} for TG');
grid on;
%saveas(gcf, fullfile(outputFolder, 'k_OV_LL_TG_vs_omega.png'));
...
function [dydt, MT_LL, MT_LS] = modelODE(~, y, p)
    ...
    MT_LS_TG   = p.K_OV_LS(1)*p.aLS*(y(7)-y(13));
    MT_LS_DG   = p.K_OV_LS(2)*p.aLS*(y(8)-y(14));
    MT_LS_MG   = p.K_OV_LS(3)*p.aLS*(y(9)-y(15));
    MT_LS_G    = p.K_OV_LS(4)*p.aLS*(y(10)-y(16));
    MT_LS_FAME = p.K_OV_LS(5)*p.aLS*(y(11)-y(17));
    MT_LS_MeOH = p.K_OV_LS(6)*p.aLS*(y(12)-y(18));

    %TG phase balance
    dNTG_o_dt= (p.Fv_TG*p.Cin(1)-p.Fv_TG*y(1)-MT_LL_TG)/p.V_MeOH; 

    dNDG_o_dt= (                -p.Fv_TG*y(2)-MT_LL_DG)/p.V_MeOH;  

    dNMG_o_dt= (                -p.Fv_TG*y(3)-MT_LL_MG)/p.V_MeOH;

    dNG_o_dt=  (                -p.Fv_TG*y(4)-MT_LL_G)/p.V_MeOH;

    dNF_o_dt=  (                -p.Fv_TG*y(5)-MT_LL_FAME)/p.V_MeOH;

    dNME_o_dt= (                -p.Fv_TG*y(6)-MT_LL_MeOH)/p.V_MeOH;

    % Methanol phase

    dNTG_m_dt= (                   -p.Fv_MeOH*y(7) +MT_LL_TG -MT_LS_TG)/p.V_TG;

    dNDG_m_dt= (                   -p.Fv_MeOH*y(8) +MT_LL_DG -MT_LS_DG)/p.V_TG;
   
    dNMG_m_dt= (                   -p.Fv_MeOH*y(9) +MT_LL_MG -MT_LS_MG)/p.V_TG; 
    
    dNG_m_dt=  (                   -p.Fv_MeOH*y(10)+MT_LL_G -MT_LS_G)/p.V_TG;
    
    dNF_m_dt=  (                   -p.Fv_MeOH*y(11)+MT_LL_FAME -MT_LS_FAME)/p.V_TG;
    
    dNME_m_dt= (p.Fv_MeOH*p.Cin(12)-p.Fv_MeOH*y(12)+MT_LL_MeOH-MT_LS_MeOH)/p.V_TG;
